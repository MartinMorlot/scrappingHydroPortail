...
matplot(dataFrame_normalized_data$date[which(format(dataFrame_normalized_data$date, "%Y") > 2015)],
        dataFrame_normalized_data[c(which(format(dataFrame_normalized_data$date, "%Y") > 2015)),-c(1)],
         type='l', ylab='DeltaH', xlab='Date', ylim=c(-3,0))

library(ggcorrplot)

data_post_2015 <- dataFrame_normalized_data[which(format(dataFrame_normalized_data$date, "%Y") > 2015),]
...
pdf("Correlations_normalize_over_06_close_new.pdf")
object_count <- 1
...
writeVector(stations_sel, "station_good_for_correlation.gpkg", overwrite=T)
...
writeVector(good_cor_lines, "good_correlation_normalized.gpkg", overwrite=T)
